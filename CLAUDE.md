...
| Teknisi | Create/edit devices and maintenance; **soft-delete** Peralatan & Pemeliharaan (see below) |
...
- **Soft-delete vs hard-delete gate are two different permissions, don't conflate them.** `can_delete`/`require_can_delete` (`devices/permissions.py`) is **hard delete, superuser only** — fiber optic, foto lapangan, icon, device eviden/event, asesmen optik; these have no recovery path. `can_soft_delete`/`require_can_soft_delete` is **soft delete, superuser + Teknisi** (`UserProfile.bisa_soft_delete`) — currently `device_delete` (Peralatan) and `maintenance_delete` (Pemeliharaan); the row is flagged `is_deleted=True` + `deleted_by`, never actually removed, so an accidental delete is recoverable from Admin (`is_deleted` filter + "Pulihkan data yang terhapus" action on both `DeviceAdmin` and `MaintenanceAdmin`). Templates read the matching context flag from `devices.context_processors.user_permissions` — `user_can_delete` for the hard-delete buttons, `user_can_soft_delete` for the Peralatan/Pemeliharaan delete buttons — never reuse one for the other's button. `maintenance.Maintenance` additionally defines `objects` as a manager that auto-filters `is_deleted=True` (unlike `devices.Device`, which filters `is_deleted=False` explicitly per query) because Maintenance is queried from far more places (health_index, jadwal, gudang, devices) — a manager-level filter means none of those call sites can forget it, including `get_object_or_404(Maintenance, pk=...)` in edit/detail/PDF views. The one gap a manager can't close: JOIN-based lookups across the relation (e.g. `Device.objects.filter(maintenance__status=...)`) bypass the manager entirely, so those add `maintenance__is_deleted=False` explicitly (see `devices/views.py` dashboard aggregate and `jadwal/models.py::get_progress()`) — check for this pattern before trusting a new cross-relation Maintenance filter. To see/restore soft-deleted Maintenance rows, query `Maintenance.semua_objects` (unfiltered), never `Maintenance.objects`.
