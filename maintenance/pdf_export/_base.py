...
def draw_header(c, kind):
    ...
    if os.path.exists(petir):
        lh = HDR_H - 2*mm
        lw = lh * (474/714)
        c.drawImage(petir, W-MR-lw, logo_y+1*mm, width=lw, height=lh,
...
def draw_pengesahan(c, y, info, signatures=None):
    ...
    hdr = Table([[_p('Mengetahui,',7,True,C_WHITE),_p('Pelaksana Pemeliharaan',7,True,C_WHITE)]],
                colWidths=[AM_W,TEK_W])
    hdr.setStyle(TableStyle([('BACKGROUND',(0,0),(-1,-1),C_BLUE_DARK),
        ('TOPPADDING',(0,0),(-1,-1),3.5),('BOTTOMPADDING',(0,0),(-1,-1),3.5),
        ('LEFTPADDING',(0,0),(-1,-1),6)]))
    y = _draw(c, hdr, ML, y)
    ttl = Table([[_p('Asisten Manager Operasi',7,True,C_GRAY_TXT),_p('',7)]],
                colWidths=[AM_W,TEK_W])
    ttl.setStyle(TableStyle([('BOX',(0,0),(-1,-1),0.4,C_GRAY_LINE),('INNERGRID',(0,0),(-1,-1),0.3,C_GRAY_LINE),
    ...
    spx=6*mm; spy=4*mm; sw=AM_W-2*spx; sh=zh-2*spy; sx=ML+spx; syb=zb+spy
    sig_path = sigs.get('asisten_manager','')
    if sig_path and os.path.exists(sig_path):
        pad=3*mm
        c.drawImage(sig_path,sx+pad,syb+pad,width=sw-2*pad,height=sh-2*pad,
                    preserveAspectRatio=True,anchor='c',mask='auto')
    else:
        c.saveState(); c.setFont('Helvetica',6.5); c.setFillColor(colors.HexColor('#CBD5E1'))
        lbl='Tanda Tangan'; lw=c.stringWidth(lbl,'Helvetica',6.5)
        c.drawString(sx+(sw-lw)/2, syb+sh/2-3, lbl); c.restoreState()
    if technicians:
        ...
        for i,name in enumerate(names):
            c.drawString(ML+AM_W+6*mm, zt-(12+i*7)*mm, f'{i+1}.  {name}')
    y = zb
    bot = Table([[_p(f'( {"................................................" if not (info.get("signed_by") or "").strip() else info["signed_by"]} )',7.5,False,C_BLACK,TA_CENTER),_p('',7.5)]],
                colWidths=[AM_W,TEK_W])
    bot.setStyle(TableStyle([('BOX',(0,0),(-1,-1),0.4,C_GRAY_LINE),('INNERGRID',(0,0),(-1,-1),0.3,C_GRAY_LINE),
