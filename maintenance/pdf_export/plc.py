"""
pdf_export/plc.py
Konten PDF untuk perangkat PLC (Power Line Carrier).
"""
from reportlab.lib.units import mm
from reportlab.platypus import Table
from ._base import (
    _p, _val, _sc, _sbg, _grid, _sec, _draw,
    C_BLUE_MID, C_GRAY_HEAD, C_WHITE, C_GRAY_TXT, C_BLACK,
    TA_CENTER, CW, H, ML
)
...
def render(c, y, data):
    ...
    def mkcol(rows):
        t = Table([[_p(l, 7.5), _sc(v)] for l, v in rows],
                  colWidths=[LBL, VAL])
        ex = []
    ...
    ub2.setStyle(_grid([
        ('BACKGROUND',(0,i),(-1,i), C_GRAY_HEAD if i%2==0 else C_WHITE)
    ...
    ct = Table([[_p(_val(cat), 7.5, False,
                    C_GRAY_TXT if not cat else C_BLACK)]], colWidths=[CW])
