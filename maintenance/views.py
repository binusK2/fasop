        ...
        if old_status != 'Done' and maintenance.status == 'Done':
            try:
                ...
                notif_ke_am(
                    ...
                    level  = 'info',
                    url    = reverse('maintenance_view', kwargs={'pk': maintenance.pk}),
                    device = maintenance.device,
                )
            except Exception:
                pass
...
def maintenance_detail(request, pk):
    ...
    return render(request, 'maintenance/maintenance_detail.html', {
        ...
        'tp_skema_list': ([
            {'n': n,
             'command':      getattr(tp_detail, f'skema_{n}_command', ''),
             'send_teg':     getattr(tp_detail, f'skema_{n}_send_teg', ''),
             'send_pol':     getattr(tp_detail, f'skema_{n}_send_pol', ''),
             'receive_teg':  getattr(tp_detail, f'skema_{n}_receive_teg', ''),
             'receive_pol':  getattr(tp_detail, f'skema_{n}_receive_pol', ''),
             # lama — tetap dikirim untuk backward-compat detail lama
            ...
            for n in range(1, (tp_detail.jumlah_skema or 4) + 1)
            if n <= 4
        ]) if tp_detail else [],
        'tp_uji_list': ([
            item
            for n in range(1, (tp_detail.jumlah_skema or 4) + 1) if n <= 4
            for item in [
                (f'Send Command {n}',    getattr(tp_detail, f'skema_{n}_send_result', '')),
                (f'Receive Command {n}', getattr(tp_detail, f'skema_{n}_receive_result', '')),
            ]
        ]) if tp_detail else [],
...
def blank_maintenance_pdf(request, device_id):
    ...
    data = {
        ...
        'freq_relay': {}, 'corrective': {}, 'master_trip': {},
    }
...
def export_maintenance_pdf(request, pk):
    ...
    data = {
        ...
        'tp': {
            ...
            'skema_1_send_teg':      _g(tp_detail, 'skema_1_send_teg', ''),
            'skema_1_send_pol':      _g(tp_detail, 'skema_1_send_pol', ''),
            'skema_1_receive_teg':   _g(tp_detail, 'skema_1_receive_teg', ''),
            'skema_1_receive_pol':   _g(tp_detail, 'skema_1_receive_pol', ''),
            ...
            'skema_2_send_teg':      _g(tp_detail, 'skema_2_send_teg', ''),
            'skema_2_send_pol':      _g(tp_detail, 'skema_2_send_pol', ''),
            'skema_2_receive_teg':   _g(tp_detail, 'skema_2_receive_teg', ''),
            'skema_2_receive_pol':   _g(tp_detail, 'skema_2_receive_pol', ''),
            ...
            'skema_3_send_teg':      _g(tp_detail, 'skema_3_send_teg', ''),
            'skema_3_send_pol':      _g(tp_detail, 'skema_3_send_pol', ''),
            'skema_3_receive_teg':   _g(tp_detail, 'skema_3_receive_teg', ''),
            'skema_3_receive_pol':   _g(tp_detail, 'skema_3_receive_pol', ''),
            ...
            'skema_4_send_teg':      _g(tp_detail, 'skema_4_send_teg', ''),
            'skema_4_send_pol':      _g(tp_detail, 'skema_4_send_pol', ''),
            'skema_4_receive_teg':   _g(tp_detail, 'skema_4_receive_teg', ''),
            'skema_4_receive_pol':   _g(tp_detail, 'skema_4_receive_pol', ''),
            ...
            if status_perbaikan == 'selesai':
                try:
                    ...
                    notif_ke_am(
                        ...
                        level  = 'success',
                        url    = reverse('maintenance_view', kwargs={'pk': maint.pk}),
                        device = device,
                    )
                except Exception:
                    pass
