...
Hitung kinerja (uptime %) harian titik ANALOG dari OFDB (dbup2bmakasar.scd_his_analog,
dibaca read-only) dan simpan ke KinerjaAnalogHarian (PostgreSQL).
...
class Command(BaseCommand):
    help = 'Hitung kinerja harian titik ANALOG dari OFDB -> KinerjaAnalogHarian'
                ...
                for point_number, path1, path2, path3 in points:
                    try:
                        jlh, uptime, performance = ofdb.compute_point_kinerja(
                            cursor, 'scd_his_analog', point_number, day_start, day_end
                        )
                        ...
                        if dry_run:
                            ...
                            KinerjaAnalogHarian.objects.update_or_create(
                        ...
                        logger.error(f'sync_kinerja_analog error [{point_number}] {tanggal}: {e}')
                        error += 1
