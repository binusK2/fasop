...
echo "    Belum disiapkan script ini sama sekali. Setelah coturn jalan, isi"
echo "    kredensial yang SAMA PERSIS di 2 tempat:"
echo "      - $ENV_FILE                 -> WEBRTC_ICE_SERVERS"
echo "      - $OUTPUT -> webrtcICEServers2 (password:)"
echo ""
echo " 2. $ENV_FILE"
echo "    -> MEDIAMTX_WHIP_URL / MEDIAMTX_WHEP_URL: isi host:port PUBLIK"
echo "       MediaMTX (bukan localhost, kecuali Django & MediaMTX satu server)."
echo ""
echo " 3. $OUTPUT"
echo "    -> webrtcAllowOrigin: ganti dari \"*\" ke domain FASOP produksi."
echo "    -> webrtcEncryption: wajib \"strict\" + cert/key TLS di produksi"
echo "       (browser HP blokir getUserMedia di halaman non-HTTPS)."
echo "    -> authHTTPAddress & runOnRecordSegmentComplete: masih pakai"
echo "       127.0.0.1:8000 — ganti kalau MediaMTX ada di server TERPISAH"
echo "       dari Django/gunicorn."
echo ""
