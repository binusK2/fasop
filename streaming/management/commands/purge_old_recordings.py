...
Hapus file rekaman live streaming (video + klip audio talkback pengawas,
kalau ada) yang sudah lewat masa retensi (STREAMING_RECORDING_RETENTION_DAYS,
default 7 hari) dihitung sejak sesi live diakhiri (ended_at).
...
_FIELDS = ['recording_path', 'talkback_recording_path']
...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        sessions = LiveSession.objects.filter(
            status='ended',
            ended_at__lt=batas,
        ).exclude(recording_path='', talkback_recording_path='')

        total = 0
        for session in sessions:
            update_fields = []
            for field in _FIELDS:
                path = getattr(session, field)
                if not path:
                    continue
                if dry_run:
